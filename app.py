...
APP_BUILD = "2026-01-07-reason-dropdown-plus-capabilities-v1"
...
# Flattened labels for Streamlit dropdown
REFERRAL_REASON_OPTIONS = []
REFERRAL_REASON_OPTION_TO_META = {}
for _cat, _items in REFERRAL_REASON_CATALOG.items():
    for _item in _items:
        _label = f"{_cat} — {_item}"
        REFERRAL_REASON_OPTIONS.append(_label)
        REFERRAL_REASON_OPTION_TO_META[_label] = {"category": _cat, "reason": _item}
...
def analyze_referral_reasons(referrals):
    ...
    reasons_data = {
        ...
        'special_test_or_intervention': 0,
        'capabilities': {}
    }
    ...
    for ref in referrals:
        try:
            ...
            if ref_reasons.get('specialTest'):
                reasons_data['special_test_or_intervention'] += 1
...
tabs = st.tabs(["Referrer","Ambulance / EMT","Receiving Hospital","Government Analytics","Data / Admin","Facility Admin","Patient / Citizen"])
...
with tabs[0]:
    ...
    if referrer_role == "Doctor/Physician":
        ...
        # ANM/ASHA/EMT: Optional ICD with prominent free-text (clinical summary)
        st.markdown("**Provisional diagnosis / clinical summary (optional)**", unsafe_allow_html=True)
        
        # Free-text reason (primary for non-doctors)
        referral_reason = st.text_area("Clinical summary (optional)", 
    ...
    # -------------------------
    # Capabilities needed (dropdown-based selection)
    # -------------------------
    
    # -------------------------
    # Reason for Referral (structured dropdown)
    # -------------------------
    st.subheader('Reason for Referral (select all that apply)')
    st.caption('Choose one or more reasons for this transfer. This is separate from the capabilities needed below.')

    selected_reason_options = st.multiselect(
        'Reason(s) for referral',
        options=REFERRAL_REASON_OPTIONS,
        default=[],
        key='rf_reason_options'
    )

    reason_other_details = ''
    if any(opt.endswith('— Other / not listed') for opt in selected_reason_options):
        reason_other_details = st.text_input('If Other, specify', key='rf_reason_other')

    # Backwards-compatible flags for legacy analytics (derived)
    bed_or_icu_unavailable = any('ICU / Critical-care resource unavailable' in opt or 'bed unavailable' in opt for opt in selected_reason_options)
    special_test_or_intervention = any('Diagnostics / Imaging unavailable' in opt or 'Procedures / Interventions unavailable' in opt for opt in selected_reason_options)

    st.subheader("Capabilities needed (select all that apply)")
    st.caption("Select what the receiving facility must have for safe definitive care.")
    ...
    # Procedures / Interventions / Resources
    PROC_LIST = [
        "OT / Surgery (OR)",
        "Neuro OT",
        ...
        "Ventilator",
        "NIV / CPAP / BiPAP",
        "Oxygen / pipeline support",
        "Blood bank / blood products",
        "Cath Lab / PCI",
        "Thrombolysis (Stroke)",
        "Dialysis / RRT",
        "Endoscopy / Bronchoscopy",
        "Central line / advanced airway support",
        "Isolation / infection control",
        "Other / Not listed",
    ]

    # Defaults from any auto-suggestion logic (if present)
    suggested = set(st.session_state.get("auto_suggested_caps", []) or [])
    default_depts, default_icus, default_diags, default_procs = [], [], [], []

    # Suggested -> Departments
    if "Cardiology" in suggested: default_depts.append("Cardiology")
    if "Paediatrics" in suggested: default_depts.append("Paediatrics")
    if "OBGYN" in suggested: default_depts.append("Obstetrics & Gynaecology")
    if "Orthopaedics" in suggested: default_depts.append("Orthopaedics")
    if "GeneralSurgery" in suggested: default_depts.append("General Surgery")
    if "Neurosurgery" in suggested: default_depts.append("Neurology / Neurosurgery")

    # Suggested -> ICUs
    if "ICU" in suggested: default_icus.append("ICU")
    if "NICU" in suggested: default_icus.append("NICU")

    # Suggested -> Diagnostics
    if "CT" in suggested: default_diags.append("CT scan")
    if "MRI" in suggested: default_diags.append("MRI")

    # Suggested -> Procedures/Resources
    if "OR" in suggested: default_procs.append("OT / Surgery (OR)")
    if "Ventilator" in suggested: default_procs.append("Ventilator")
    if "BloodBank" in suggested: default_procs.append("Blood bank / blood products")
    if "CathLab" in suggested: default_procs.append("Cath Lab / PCI")
    if "Dialysis" in suggested: default_procs.append("Dialysis / RRT")
    if "Thrombolysis" in suggested: default_procs.append("Thrombolysis (Stroke)")

    dept_needed = st.multiselect("Specialist / Department", DEPT_LIST, default=default_depts, key="need_depts")
    icu_needed = st.multiselect("ICU / Critical-care", ICU_LIST, default=default_icus, key="need_icus")
    diag_needed = st.multiselect("Diagnostics / Imaging / Equipment", DIAG_LIST, default=default_diags, key="need_diags")
    proc_needed = st.multiselect("Procedures / Interventions / Resources", PROC_LIST, default=default_procs, key="need_procs")

    # Map selected items -> facility capability keys used by matching logic
    DEPT_TO_CAPS = {
    ...
    cap_keys = set()
    for d in dept_needed: cap_keys.update(DEPT_TO_CAPS.get(d, []))
    for i in icu_needed: cap_keys.update(ICU_TO_CAPS.get(i, []))
    for d in diag_needed: cap_keys.update(DIAG_TO_CAPS.get(d, []))
    for p in proc_needed: cap_keys.update(PROC_TO_CAPS.get(p, []))

    # Required by facility matching + saved into the referral payload
    need_caps = sorted(list(cap_keys))

    # Keep a verbose copy for audit/analytics (safe to store in session)
    st.session_state["need_caps_verbose"] = dict(
        departments=dept_needed,
        icu=icu_needed,
        diagnostics=diag_needed,
        procedures=proc_needed,
    )

    # Backward-compatible placeholders (old booleans removed from UI)
    ref_beds = False
    ref_tests = False
    ...
    def _save_referral(dispatch=False):
        ...
        # Capabilities are now the core driver for routing/matching
        if not need_caps:
            st.error("Please select at least one required capability for this referral (department/ICU/diagnostics/procedures).")
            return False
            ...
            reasons_df = pd.DataFrame(
                {
                    ...
                    "Count": [
                        ...
                        referral_reasons["special_test_or_intervention"],
                    ],
                }
            )
