...
# ==========================
APP_BUILD = "2026-01-05-referral-reason-dropdown-v4"
...
def render_referral_reason_picker(prefix_key: str = "ref_reason"):
    """Returns (categories_selected, reasons_selected, narrative_text)."""
    cats = list(REFERRAL_REASON_CATALOG.keys())
    default_cats = []
    categories = st.multiselect(
        "Reason category",
        options=cats,
        default=default_cats,
        help="Select one or more categories; then choose specific reasons under each category.",
        key=f"{prefix_key}_cats",
    )

    all_reasons = []
    for cat in categories:
        opts = REFERRAL_REASON_CATALOG.get(cat, [])
        picked = st.multiselect(
            f"{cat} (select one or more)",
            options=opts,
            default=[],
            key=f"{prefix_key}_{slugify_key(cat)}",
        )
        all_reasons.extend([f"{cat}: {r}" for r in picked])

    narrative = st.text_area(
        "Additional narrative (optional)",
        placeholder="Add brief symptoms/observations or any context not covered above…",
        height=80,
        key=f"{prefix_key}_narr",
    )
    return categories, all_reasons, narrative

def summarize_referral_reasons(reasons_prefixed: list[str], narrative: str) -> str:
    parts = []
    if reasons_prefixed:
        parts.extend(reasons_prefixed)
    if narrative and narrative.strip():
        parts.append(f"Notes: {narrative.strip()}")
...
st.set_page_config(
    ...
    layout="wide",
    initial_sidebar_state="expanded"  # expanded for deployment verification
)

# Build stamp visible in UI
st.sidebar.caption(f"Build: {APP_BUILD}")
st.caption(f"✅ Running build: {APP_BUILD}")  # visible on main page
...
with tabs[0]:
    ...
    st.caption("Tip: For ANM/ASHA/Nurse, Provisional Diagnosis is optional. For EMT, at least one referral reason or ICD is required.")
    ...
    if referrer_role == "Doctor/Physician":
        ...
        # Optional: structured reasons for referral (useful for audit and coordination)
        with st.expander("Reason(s) for referral (optional)"):
            _cats, _reasons, _narr = render_referral_reason_picker(prefix_key="ref_reason")
            _ = summarize_referral_reasons(_reasons, _narr)
        ...
        dx_required = (referrer_cadre == "EMT")
        if dx_required:
            st.markdown("**Reason for Referral** <span class='required'>*</span>", unsafe_allow_html=True)
            st.caption("Select at least one category + reason (or add notes) to proceed.")
        else:
            st.markdown("**Reason for Referral (optional)**", unsafe_allow_html=True)
            st.caption("Tip: select categories + sub-reasons; add narrative only if needed.")
        
        

        # Structured reasons (dropdown + subcategories)
        cats_selected, reasons_selected, reason_narrative = render_referral_reason_picker(prefix_key="ref_reason")
        referral_reason = summarize_referral_reasons(reasons_selected, reason_narrative)
        ...
        if dx_required and (not referral_reason and not chosen_icd):
            st.error("Please select a Reason category/sub-reason (or add narrative), OR select an ICD diagnosis.")
...
with c1:
    # Auto-default these based on the structured referral reasons (if selected earlier)
    cats_selected = st.session_state.get("ref_reason_cats", [])
    beds_default = "ICU / Critical-care resource unavailable" in cats_selected
    tests_default = any(c in cats_selected for c in ["Diagnostics / Imaging unavailable", "Procedures / Interventions unavailable"])

    if "ref_beds" not in st.session_state:
        st.session_state.ref_beds = beds_default
    if "ref_tests" not in st.session_state:
        st.session_state.ref_tests = tests_default

    ref_beds = st.checkbox("No ICU/bed available", key="ref_beds")
    ref_tests = st.checkbox("Special intervention/test required", key="ref_tests")
    ref_severity = True
