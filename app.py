...
# ==========================
APP_BUILD = "2026-01-05-referral-reason-dropdown-v1"
...
with tabs[0]:
    ...
    if referrer_role == "Doctor/Physician":
        ...
        # Structured reason(s) for referral (recommended)
        reason_groups = st.multiselect(
            "Select one or more reason categories",
        ...
        reason_details = {}
        for g in reason_groups:
            sub_key = f"reason_sub_{slugify_key(g)}"
            reason_details[g] = st.multiselect(
                f"{g} — details",
                options=REFERRAL_REASON_CATALOG.get(g, []),
                default=[],
                key=sub_key
            )

        # Optional narrative (for clinical context / free text)
        reason_notes = st.text_area(
            "Additional clinical details (optional)",
            placeholder="Optional: add brief symptoms, observations, timeline, vitals, or transfer context...",
            height=80
        )

        # Build a single summary string for storage + downstream analytics
        parts = []
        for g in reason_groups:
            subs = reason_details.get(g, [])
            if subs:
                parts.append(f"{g}: " + ", ".join(subs))
            else:
                parts.append(g)
        referral_reason = " | ".join(parts).strip()
        if reason_notes:
            referral_reason = (referral_reason + (" | " if referral_reason else "") + reason_notes.strip()).strip()
