...
# === REFERRAL REASON CATALOG (Structured reasons for referral) ===
REFERRAL_REASON_CATALOG = {
    ...
    "ICU / Critical care resource unavailable": [
        ...
        "Non-invasive ventilation (NIV) unavailable",
        "Oxygen supply constrained",
        "Cardiac monitor unavailable",
        "Infusion pump / syringe pump unavailable",
    ...
    "Referral for higher level of care": [
        "Need tertiary care / multispecialty",
        "Need specialist opinion urgently",
        "Need ICU/NICU stabilization",
        "Need inter-facility transfer for definitive care",
        "Need advanced monitoring / step-up care"
    ],
    "Administrative / operational": [
        "Ambulance / transport coordination",
        "Bed confirmation required",
        "Patient/family request (documented)",
        "Medico-legal / police case handling"
    ],
    "Other": [
        "Other (add in notes)"
    ],
...
with tabs[0]:
    ...
    if referrer_role == "Doctor/Physician":
        ...
        # Structured reason(s) for referral (recommended for ANM/ASHA/Nurse/EMT)
        ...
        parts = []
        for g in reason_groups:
            subs = [s for s in (reason_details.get(g) or []) if s]
            if subs:
                parts.append(f"{g}: " + "; ".join(subs))
            else:
                parts.append(g)
        referral_reason = " | ".join(parts).strip()
        if reason_notes.strip():
            referral_reason = (referral_reason + " | " if referral_reason else "") + reason_notes.strip()
