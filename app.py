...
# ==========================
# Build stamp (helps verify the deployed code)
# ==========================
APP_BUILD = "2026-01-06-provisional-optional-icd-then-reason-v9"

def slugify_key(s: str) -> str:
    return "".join([c.lower() if c.isalnum() else "_" for c in str(s)]).strip("_")

# ==========================
# Structured "Reason for Referral" catalog
# ==========================
REFERRAL_REASON_CATALOG = {
    "Specialist / Department required": [
    ...
    "ICU / Critical-care resource unavailable": [
        "ICU bed unavailable / full",
        "HDU bed unavailable",
        "Ventilator unavailable",
        "NIV / CPAP / BiPAP unavailable",
        "Oxygen / pipeline constraint",
        "Infusion pumps / syringe pumps unavailable",
        "Dialysis / RRT unavailable",
        "Isolation bed unavailable",
        "Blood products unavailable",
        "Continuous monitoring unavailable",
    ],
    "Diagnostics / Imaging unavailable": [
        "CT scan unavailable",
        "MRI unavailable",
        "USG / Ultrasound unavailable",
        "X-ray unavailable",
        "ECG unavailable",
        "2D Echo unavailable",
        "ABG unavailable",
        "Advanced lab tests unavailable",
        "Microbiology / culture unavailable",
    ],
    "Procedures / Interventions unavailable": [
        "OT unavailable",
        "Neuro OT unavailable",
        "Cath Lab / PCI unavailable",
        "Thrombolysis / Stroke pathway unavailable",
        "Emergency C-section capability unavailable",
        "Trauma surgery capability unavailable",
        "Endoscopy / Bronchoscopy unavailable",
        "Central line / advanced airway support unavailable",
        "Blood transfusion / massive transfusion support unavailable",
    ],
    "Higher level of care required": [
        "Tertiary / multispecialty care required",
        "Definitive management not available here",
        "Step-up monitoring required",
        "Specialist opinion urgently required",
        "ICU/NICU stabilisation required",
    ],
    "Administrative / Operational": [
        "Bed confirmation needed",
        "Inter-facility transfer coordination",
        "Ambulance availability constraint",
        "Family request / preference",
...
def render_reason_dropdown(dx_required: bool) -> str:
    """
    Renders structured reason selection (category + sub-reasons) and returns a compact string.
    Also stores structured selections in st.session_state for downstream use.
    """
    st.markdown("**Reason for Referral**" + (" <span class='required'>*</span>" if dx_required else " (optional)"),
                unsafe_allow_html=True)

    categories = st.multiselect(
        "Select reason category (choose one or more)",
        options=list(REFERRAL_REASON_CATALOG.keys()),
        key="reason_categories",
    )
    ...
    for cat in categories:
        subs = st.multiselect(
            f"{cat} — select details",
            options=REFERRAL_REASON_CATALOG[cat],
            key=f"reason_{slugify_key(cat)}",
        )
        structured[cat] = subs
        if subs:
            flat.append(f"{cat}: {', '.join(subs)}")
        else:
            flat.append(f"{cat}")

    notes = st.text_area(
        "Additional clinical context (optional)",
        placeholder="Optional: add brief notes (symptoms, duration, key findings)…",
        height=70,
        key="reason_notes",
    )

    st.session_state["referral_reasons_structured"] = structured
    st.session_state["referral_reason_notes"] = notes

    reason_text = " | ".join([x for x in flat if x]).strip()
    if notes:
        reason_text = (reason_text + " || " + notes) if reason_text else notes

    # Store for backward compatibility
    st.session_state["reason_for_referral"] = reason_text
    return reason_text
...
# --- Build stamp (visible on main page and sidebar) ---
st.sidebar.caption(f"Build: {APP_BUILD}")
st.caption(f"Running build: {APP_BUILD}")
...
@st.cache_data
def load_meghalaya_facilities(csv_path: "Union[str, Path]" = DATA_DIR / "meghalaya_facilities.csv"):
    """
...
def flatten_for_facility_analytics(df: pd.DataFrame) -> pd.DataFrame:
    """
    Ensure we always have 'facility' and 'triage_color' columns
    for facility performance charts, even if raw data uses 'dest'
    or nested 'triage' dicts.
    """
    ...
    if "facility" not in df.columns:
        if "dest" in df.columns:
            # most of your referrals use 'dest' as receiving facility name
            df["facility"] = df["dest"]
        elif "receiving_facility" in df.columns:
            df["facility"] = df["receiving_facility"]
        else:
            df["facility"] = "Unknown"

    # --- Triage color column ---
    if "triage_color" not in df.columns:
        if "triage" in df.columns:
            # triage is usually a dict with triage["decision"]["color"]
            def _triage_color(v):
                if isinstance(v, dict):
                    dec = v.get("decision")
                    if isinstance(dec, dict):
                        return dec.get("color")
                    # or directly triage["color"] if used
                    return v.get("color")
                return None

            df["triage_color"] = df["triage"].apply(_triage_color)
        elif "triage_decision" in df.columns:
            df["triage_color"] = df["triage_decision"]
        else:
            df["triage_color"] = None
...
def default_facilities(count: "Optional[int]" = None):
    """
    ...
    for row in facilities:
        ...
        # --- Extra capability keys for matching (departments + ICU types) ---
        # These keys are referenced by the new "Capabilities needed" drop-downs.
        caps.update({
            "Cardiology":     int(has_cardio),
            "Paediatrics":    int(has_peds),
            "OBGYN":          int(has_obgyn),
            "Orthopaedics":   int(has_ortho),
            "GeneralSurgery": int(level.lower() in ["secondary", "tertiary"]),
            "NICU":           int(has_nicu),
        })
...
with tabs[0]:
    ...
    # Cadre selector for non-doctor referrers (controls whether Provisional Dx is required)
    if referrer_role == "Doctor/Physician":
        referrer_cadre = "Doctor"
    else:
        referrer_cadre = st.selectbox("Referrer cadre", ["ANM", "ASHA", "Nurse", "EMT"], index=0)
    st.session_state.referrer_cadre = referrer_cadre
    ...
    # Free-text provisional diagnosis (optional for all cadres)
    provisional_dx_free = st.text_area(
        "Provisional diagnosis (optional)",
        value=st.session_state.get("provisional_dx_free", ""),
        placeholder="Optional: working diagnosis / suspected condition / key clinical impression",
        height=80,
        key="provisional_dx_free",
    )
    st.session_state["provisional_dx_free"] = provisional_dx_free

    # --- ICD-coded diagnosis (optional for all roles) ---
    chosen_icd = None
    row = None

    if referrer_role == "Doctor/Physician":
        st.markdown("**ICD-coded Diagnosis (optional)**")

        search_col, filter_col = st.columns([2, 1])
        with search_col:
            icd_search = st.text_input(
                "Search ICD codes",
                placeholder="Type to search diagnoses...",
                key="icd_search_doctor"
            )
        with filter_col:
            show_all = st.checkbox("Show all diagnoses", value=False, key="icd_show_all_doctor")
        ...
        if icd_search:
            icd_choices = [c for c in icd_choices if icd_search.lower() in c.lower()]

        if icd_choices:
            chosen_icd = st.selectbox(
                "ICD diagnosis (optional)",
                [""] + icd_choices,
                index=0,
                key="icd_choice_doctor"
            )
            if chosen_icd:
                row = icd_df_filt[icd_df_filt["display"] == chosen_icd].iloc[0]
        else:
            st.info("No ICD suggestions for this case type/age filter")

    else:
        # Non-doctor: keep the existing ICD dropdown (optional)
    ...
    # --- Reason for referral (dropdown with sub-categories) ---
    # Required for EMT + Doctor/Physician; optional for ANM/ASHA/Nurse
    # Reason(s) for referral should be captured for all referrers
    reason_required = True

    st.markdown(
        "### Reason(s) for Referral" + (" <span class='required'>*</span>" if reason_required else ""),
        unsafe_allow_html=True
    )
    referral_reason = render_reason_dropdown(dx_required=reason_required)

    # Additional narrative (optional)
    dx_free = st.text_input("Additional notes (optional)", "", key="dx_notes")

    # Diagnosis payload (kept compatible with downstream code)
    dx_payload = dict(
        code=(row["icd_code"] if (chosen_icd and row is not None) else ""),
        label=(row["label"] if (chosen_icd and row is not None) else ""),
        case_type=str(complaint),
        provisional_dx=provisional_dx_free,
        referral_reason=referral_reason,
        notes=dx_free
    )

    # Validation: require at least one structured reason
    if reason_required and (not referral_reason):
        st.error("Please select at least one reason for referral")
    # If "Other / not listed" is chosen, require short context note
    other_struct = st.session_state.get("referral_reasons_structured", {}).get("Other", [])
    other_note = st.session_state.get("referral_reason_notes", "")
    if ("Other / not listed" in other_struct) and (not other_note.strip()):
        st.error("Please add a short note for: Other / not listed")
    ...
    # Capabilities needed (drop-down selection)
    st.subheader("Capabilities needed (select all that apply)")

    # --- Capability catalog (UI labels) ---
    DEPT_LIST = [
    ...
    with colA:
        dept_need = st.multiselect("Specialist / Department", options=DEPT_LIST, key="need_depts")
        icu_need = st.multiselect("ICU / Critical care unit", options=ICU_LIST, key="need_icu_types")

    with colB:
        diag_need = st.multiselect("Diagnostics / Equipment", options=DIAG_LIST, key="need_diagnostics")
        proc_need = st.multiselect("Procedures / OT", options=PROC_LIST, key="need_procedures")

    # --- Store a verbose list for referral details / audit ---
    need_caps_verbose = []
    for x in dept_need:
        need_caps_verbose.append(f"Dept: {x}")
    for x in icu_need:
        need_caps_verbose.append(f"ICU: {x}")
    for x in diag_need:
        need_caps_verbose.append(f"Diag: {x}")
    for x in proc_need:
        need_caps_verbose.append(f"Proc: {x}")

    st.session_state["capabilities_requested_verbose"] = need_caps_verbose

    # --- Map UI selections to facility-matching capability keys (facility['caps']) ---
    # Keep this list aligned with facility caps keys
    need_caps = []

    def _add_cap(cap_key: str):
        if cap_key and cap_key not in need_caps:
            need_caps.append(cap_key)

    # Departments (match keys we add to facility['caps'])
    for d in dept_need:
        if d == "Cardiology":
            _add_cap("Cardiology")
        elif d in ["Neurology", "Neurosurgery"]:
            _add_cap("Neurosurgery")
        elif d == "Obstetrics & Gynaecology":
            _add_cap("OBGYN")
            _add_cap("OBGYN_OT")
        elif d == "Paediatrics":
            _add_cap("Paediatrics")
        elif d == "Neonatology / NICU":
            _add_cap("NICU")
        elif d == "Orthopaedics":
            _add_cap("Orthopaedics")
        elif d in ["General Surgery", "Trauma / Polytrauma"]:
            _add_cap("GeneralSurgery")
        elif d == "Critical Care / ICU":
            _add_cap("ICU")
    ...
    for p in proc_need:
        if p in ["OT", "Neuro OT"]:
            ...
            _add_cap("CathLab")
        elif "Thrombolysis" in p:
            _add_cap("Thrombolysis")
        elif "Emergency C-section" in p:
            _add_cap("OBGYN_OT")
        elif "Trauma surgery" in p:
            _add_cap("OR")
        elif "Advanced airway" in p:
            _add_cap("Ventilator")

    st.session_state["need_caps"] = need_caps

    # Soft guidance (do not block ANM/ASHA/Nurse)
    if (referrer_role in ["Doctor/Physician", "ANM/ASHA/EMT"]) and (referrer_role != "ANM/ASHA/EMT" or referrer_cadre == "EMT"):
        if not need_caps:
            st.warning("Select at least one capability needed to improve facility matching.")
    ...
    if st.button("Find Best Matched Facilities with Free Routing", type="primary"):
        ...
        elif referrer_role == "ANM/ASHA/EMT" and (referrer_cadre == "EMT") and not st.session_state.get("need_caps"):
            st.error("Please select required capabilities to find matching facilities")
        else:
    ...
    def _save_referral(dispatch=False):
        ...
        if referrer_role == "Doctor/Physician" and dx_payload is None:
            ...
            return False
        elif referrer_role == "ANM/ASHA/EMT" and (referrer_cadre == "EMT") and not dx_payload.get("label"):
        ...
        ref = dict(
            ...
            referrer=dict(name=r_name, facility=r_fac, role=referrer_role, cadre=referrer_cadre),
            ...
            reasons=dict(
                severity=True,
                bedOrICUUnavailable=False,  # not captured in current UI
                specialTest=bool(st.session_state.get('need_diagnostics') or st.session_state.get('need_procedures')),
