...
# ==========================
# Build stamp (helps verify deployed code)
# ==========================
APP_BUILD = "2026-01-05-referral-reason-dropdown-v5"
...
# ==========================
# Structured "Reason for Referral" catalog (for dropdowns)
# ==========================
...
# Map selected reasons to existing capability keys used by facility matching
REFERRAL_REASON_TO_CAPS = {
    "ICU bed unavailable / full": ["ICU"],
    "HDU bed unavailable": ["ICU"],
    "Ventilator unavailable": ["Ventilator"],
    "NIV / CPAP / BiPAP unavailable": ["Ventilator"],
    "Dialysis / RRT unavailable": ["Dialysis"],
    "Blood products unavailable": ["BloodBank"],
    "CT scan unavailable": ["CT"],
    "OT unavailable": ["OR"],
    "Emergency C-section capability unavailable": ["OBGYN_OT", "OR"],
    "Neuro OT unavailable": ["Neurosurgery", "OR"],
    "Cath Lab / PCI unavailable": ["CathLab"],
    "Thrombolysis / Stroke pathway unavailable": ["Thrombolysis"],
    "Trauma surgery capability unavailable": ["OR"],
}

def derive_caps_from_referral_reasons(structured: dict) -> list:
    caps = []
    if not structured:
        return caps
    for _, subs in structured.items():
        for s in subs or []:
            for c in REFERRAL_REASON_TO_CAPS.get(s, []):
                if c not in caps:
                    caps.append(c)
    # Only keep caps that exist in REQ_CAPS (avoid breaking facility matching)
    caps = [c for c in caps if c in REQ_CAPS]
    return caps
...
with tabs[0]:
    ...
    if referrer_role == "Doctor/Physician":
        ...
        # Structured reason selection (dropdown categories + sub-reasons)
        st.markdown("Describe the reason for referral")
        reason_categories = st.multiselect(
            "Reason category (select one or more)",
            options=list(REFERRAL_REASON_CATALOG.keys()),
            key="reason_categories",
        )
        structured_reasons = {}
        for cat in reason_categories:
            structured_reasons[cat] = st.multiselect(
                f"{cat} — details",
                options=REFERRAL_REASON_CATALOG.get(cat, []),
                key=f"reason_{slugify_key(cat)}",
            )

        # Optional narrative (kept small; structured reasons are primary)
        narrative_reason = st.text_area(
            "Additional narrative (optional)",
            placeholder="Optional brief note (e.g., key symptoms, timing, constraints)...",
            height=60,
            key="reason_narrative",
        )

        # Build a compact string summary for downstream fields/validation
        flat_parts = []
        for cat, subs in structured_reasons.items():
            if subs:
                flat_parts.append(f"{cat}: {', '.join(subs)}")
        referral_reason = " | ".join(flat_parts).strip()
        if narrative_reason and narrative_reason.strip():
            referral_reason = (referral_reason + (" | " if referral_reason else "") + narrative_reason.strip()).strip()

        # Persist structured reasons + derived caps for later capability preselect
        st.session_state["structured_referral_reasons"] = structured_reasons
        st.session_state["caps_from_referral_reasons"] = derive_caps_from_referral_reasons(structured_reasons)
        ...
        if dx_required and (not referral_reason and not chosen_icd):
            st.error("Please select a reason for referral (from dropdown) or select an ICD diagnosis")
    ...
    # Merge auto-suggestions from structured referral reasons (non-doctors and doctors)
    extra_caps = st.session_state.get("caps_from_referral_reasons", []) or []
    for c in extra_caps:
        if c not in auto_suggested_caps:
            auto_suggested_caps.append(c)
    if extra_caps:
        st.info(f"**From referral reasons:** {', '.join(extra_caps)}")
    need_caps = []
    if ref_tests:
