...
st.set_page_config(page_title="AHECN – Streamlit MVP", layout="wide")
...
def tri_color(v):
    ...
    if v["complaint"]=="Maternal" and v["rf_pph"]:
        return "RED"
    return "RED"
...
# ---------- Demo facilities ----------
DEFAULT_FAC = [
  dict(name="District Hospital X", lat=26.8467, lon=80.9462, ICU_open=2, acceptanceRate=0.86, zone="Central",
       caps=dict(ICU=1,Ventilator=1,BloodBank=1,OR=1,CT=1,Thrombolysis=1,OBGYN_OT=0,CathLab=0,Dialysis=1,Neurosurgery=0)),
  dict(name="Women & Child Hospital", lat=26.86, lon=80.98, ICU_open=1, acceptanceRate=0.78, zone="North",
       caps=dict(ICU=1,Ventilator=1,BloodBank=1,OR=1,CT=0,Thrombolysis=0,OBGYN_OT=1,CathLab=0,Dialysis=0,Neurosurgery=0)),
  dict(name="Neuro Medical Center", lat=26.87, lon=80.93, ICU_open=0, acceptanceRate=0.72, zone="Central",
       caps=dict(ICU=1,Ventilator=1,BloodBank=0,OR=0,CT=1,Thrombolysis=1,OBGYN_OT=0,CathLab=0,Dialysis=0,Neurosurgery=1)),
  dict(name="City Cardiac & Cath Lab", lat=26.90, lon=80.95, ICU_open=3, acceptanceRate=0.90, zone="South",
       caps=dict(ICU=1,Ventilator=1,BloodBank=0,OR=0,CT=0,Thrombolysis=0,OBGYN_OT=0,CathLab=1,Dialysis=0,Neurosurgery=0)),
  dict(name="Trauma & Ortho Center", lat=26.83, lon=80.91, ICU_open=1, acceptanceRate=0.81, zone="South",
...
st.title("AHECN – Streamlit MVP")
tabs = st.tabs(["Referrer", "Ambulance", "Receiving Hospital", "Government", "Data"])
...
with tabs[0]:
    ...
    with c1:
        ...
        p_age  = st.number_input("Age", 0, 120, 34)
    ...
    with v1:
        hr   = st.number_input("HR", 0, 250, 110)
        sbp  = st.number_input("SBP", 0, 300, 95)
        rr   = st.number_input("RR", 0, 80, 24)
        temp = st.number_input("Temp °C", 30.0, 43.0, 38.2, step=0.1)
    with v2:
        spo2 = st.number_input("SpO₂ %", 50, 100, 93)
        avpu = st.selectbox("AVPU", ["A","V","P","U"])
        complaint = st.selectbox("Chief complaint", ["Maternal","Trauma","Stroke","Cardiac","FeverConfusion","Sepsis","Other"])
        rf_sbp = st.checkbox("Flag: SBP <90", value=False)
    with v3:
        rf_spo2 = st.checkbox("Flag: SpO₂ <90%", value=False)
        rf_avpu = st.checkbox("Flag: AVPU ≠ A", value=(avpu!="A"))
    ...
    with c1:
        ...
        ref_tests= st.checkbox("Reason: Special intervention/test", value=False)
    need_caps = []
    if ref_tests:
        ...
        for i,cap in enumerate(CAP_LIST):
            if cap_cols[i%5].checkbox(cap, value=False, key=f"cap_{cap}"):
                need_caps.append(cap)

    if st.button("Find matched facilities"):
        fac = st.session_state.facilities
        rows = []
        for f in fac:
            if not all(f["caps"].get(c,0)==1 for c in need_caps): 
                continue
            km = dist_km(p_lat, p_lon, f["lat"], f["lon"])
            score = composite_score(f, km, need_caps)
            rows.append(dict(name=f["name"], km=round(km,1), ICU_open=f["ICU_open"],
                             accept=int(round(f["acceptanceRate"]*100,0)), score=int(round(score*100,0))))
        if rows:
            ...
            if st.button("Create referral"):
                ref = dict(
                    ...
                    dest=chosen, times=dict(first_contact_ts=time.time(), decision_ts=time.time()), status="PREALERT"
                )
                st.session_state.referrals.insert(0, ref)
                st.success(f"Referral {ref['id']} created → {chosen}")
...
with tabs[1]:
    ...
    for r in active:
        ...
        if c1.button("Dispatch", key=f"d_{r['id']}"): r["times"]["dispatch_ts"]=time.time(); r["status"]="DISPATCHED"
        if c2.button("Arrive scene", key=f"a_{r['id']}"): r["times"]["arrive_scene_ts"]=time.time(); r["status"]="ARRIVE_SCENE"
        if c3.button("Depart scene", key=f"ds_{r['id']}"): r["times"]["depart_scene_ts"]=time.time(); r["status"]="DEPART_SCENE"
        if c4.button("Arrive dest", key=f"ad_{r['id']}"): r["times"]["arrive_dest_ts"]=time.time(); r["status"]="ARRIVE_DEST"
        if c5.button("Handover", key=f"h_{r['id']}"): r["times"]["handover_ts"]=time.time(); r["status"]="HANDOVER"
        st.caption(f"Stage: {r['status']}")

# ----- Receiving Hospital -----
with tabs[2]:
    st.subheader("Receiving Hospital")
    fac_names = [f["name"] for f in st.session_state.facilities]
    st.session_state.active_fac = st.selectbox("Facility", fac_names, index=0)
    ...
    for r in incoming:
        st.write(f"**{r['patient']['name']}** — {r['triage']['complaint']} • **{r['triage']['decision']['color']}**")
...
"""
            st.code(isbar)
            c1,c2 = st.columns(2)
            if c1.button("Accept", key=f"accept_{r['id']}"):
                r["status"]="ARRIVE_DEST"; r["times"]["arrive_dest_ts"]=time.time(); st.success("Accepted")
            if c2.button("Request divert", key=f"div_{r['id']}"):
                r["reasons"]["bedOrICUUnavailable"]=True; r["status"]="PREALERT"; st.warning("Requested divert")

# ----- Government -----
with tabs[3]:
    st.subheader("Government KPIs")
    tri_filter = st.selectbox("Triage filter", ["All","RED","YELLOW","GREEN"], index=0)
    cond_filter= st.selectbox("Condition filter", ["All","Maternal","Trauma","Stroke","Cardiac","FeverConfusion","Sepsis","Other"], index=0)
    ...
    def minutes(a,b): return int((b-a)/60) if (a and b) else None
    ...
    k1,k2 = st.columns(2); k1.metric("% RED ≤60 min", f"{pct_red_60}%"); k2.metric("% Dispatch ≤10 min", f"{pct_disp_10}%")
    st.markdown("**Top receiving facilities (count)**")
    if data:
        s = pd.Series([r["dest"] for r in data]).value_counts().reset_index()
        s.columns = ["Facility","Cases"]; st.dataframe(s, use_container_width=True)
    else:
        st.caption("No data")
...
with tabs[4]:
    ...
    st.download_button("Download JSON", data=json.dumps(dict(referrals=st.session_state.referrals, facilities=st.session_state.facilities), indent=2),
                       file_name="ahecn_streamlit_data.json", mime="application/json")
    if st.button("Seed demo"):
        st.session_state.referrals.clear()
        for i in range(6):
            st.session_state.referrals.append(dict(
                id=f"RSEED{i}",
                patient=dict(name=f"Pt{i}", age=30+i, sex="Female" if i%2==0 else "Male", id="", location=dict(lat=26.85,lon=80.95)),
                referrer=dict(name="Dr. Demo", facility="PHC Demo"),
                provisionalDx="PPH" if i%2==0 else "—",
                triage=dict(
                    complaint=np.random.choice(["Maternal","Trauma","Stroke","Cardiac","Sepsis"]),
                    decision=dict(color=np.random.choice(["RED","YELLOW","GREEN"])),
                    hr=100, sbp=100, rr=20, temp=37, spo2=95, avpu="A"
                ),
                clinical=dict(summary="Seed case"),
                reasons=dict(severity=True, bedOrICUUnavailable=False, specialTest=False, requiredCapabilities=[]),
                dest=np.random.choice([f["name"] for f in st.session_state.facilities]),
                times=dict(first_contact_ts=time.time(), decision_ts=time.time(), dispatch_ts=time.time()+60),
                status="DISPATCHED"
            ))
        st.success("Seeded")
