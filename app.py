...
# ==========================
# Referral state machine + audit helpers (investor-grade traceability)
# ==========================
STATUS_TS_FIELD = {
    "DISPATCHED": "dispatch_ts",
    "ENROUTE_SCENE": "enroute_ts",
    "ARRIVE_SCENE": "arrive_scene_ts",
    "DEPART_SCENE": "depart_scene_ts",
    "ARRIVE_DEST": "arrive_dest_ts",
    "HANDOVER": "handover_ts",
}

ALLOWED_TRANSITIONS = {
    "PREALERT": {"DISPATCHED"},
    "DISPATCHED": {"ENROUTE_SCENE", "ARRIVE_SCENE"},   # allow skip for demo
    "ENROUTE_SCENE": {"ARRIVE_SCENE"},
    "ARRIVE_SCENE": {"DEPART_SCENE"},
    "DEPART_SCENE": {"ARRIVE_DEST"},
    "ARRIVE_DEST": {"HANDOVER"},
    "HANDOVER": set(),
}

def _ensure_referral_min_schema(ref: dict) -> dict:
    """Best-effort normalizer for imports/demo-seeded cases."""
    ref = dict(ref or {})
    ref.setdefault("id", f"R{int(time.time())}")
    ref.setdefault("patient", {})
    ref.setdefault("triage", {})
    ref["triage"].setdefault("decision", {"color": "GREEN"})
    ref.setdefault("times", {})
    ref.setdefault("reasons", {})
    ref.setdefault("audit_log", [])
    return ref

def log_audit(ref: dict, action: str, user: str = "system", details: dict | None = None) -> None:
    ref = _ensure_referral_min_schema(ref)
...
def apply_transition(
    ...
    """Update status + timestamps consistently; always writes to audit_log."""
    ref = _ensure_referral_min_schema(ref)
    old = str(ref.get("status", "PREALERT")).strip().upper()

    new_status = str(new_status).strip().upper()
    if not force and old in ALLOWED_TRANSITIONS and new_status not in ALLOWED_TRANSITIONS[old]:
        msg = f"Blocked transition {old} → {new_status}"
        log_audit(ref, "STATUS_CHANGE_BLOCKED", user=user, details={"from": old, "to": new_status, "reason": msg})
        return False, msg

    ref["status"] = new_status

    # timestamp mapping (only set once)
    ts_field = STATUS_TS_FIELD.get(new_status)
    if ts_field:
        ref.setdefault("times", {})
        ref["times"].setdefault(ts_field, now_ts())

    log_audit(ref, action, user=user, details={"from": old, "to": new_status, **(details or {})})
    return True, f"{old} → {new_status}"

def normalize_referral(ref: dict) -> dict:
    """Public normalizer used on import/demo injections."""
    r = _ensure_referral_min_schema(ref)
    # Normalise decision payload shape
    dec = r.get("triage", {}).get("decision", {})
    if isinstance(dec, str):
        r["triage"]["decision"] = {"color": dec}
    elif isinstance(dec, dict):
        r["triage"]["decision"].setdefault("color", dec.get("colour", "GREEN"))
    # Ensure status is uppercase string
...
def _triage_rank(color: str) -> int:
    c = (color or "").upper()
    return {"GREEN": 0, "YELLOW": 1, "ORANGE": 2, "RED": 3}.get(c, 0)
...
def triage_with_ai(vitals: dict, context: dict, complaint: str, mode: str = "rules"):
    """
    Rules-first triage with optional AI overlay.
    - mode='rules': ignore AI
    - mode='ai': use AI if available; fall back to rules
    - mode='hybrid': conservative merge (uses worse severity of rules vs AI)
    """
    ...
    ai_color = None
    probs = None
    note = "Rules-only (default)"

    if model is not None and mode in {"ai", "hybrid"}:
        ai_color, probs = ai_triage_predict(vitals, context, complaint)
        if ai_color:
            note = "AI prediction available"
        else:
            note = "AI enabled but prediction unavailable; using rules"

    if mode == "ai" and ai_color:
        final = ai_color
    elif mode == "hybrid" and ai_color:
        final = _worst_color(rules_color, ai_color)  # safety-first merge
        note = f"Hybrid (worst-of rules vs AI): rules={rules_color}, ai={ai_color}"
    else:
        final = rules_color

    return final, {
        "mode": mode if (model is not None and mode in {"ai", "hybrid"}) else "rules",
...
"""
                    ...
                    if c3.button("Reject case", key=f"rej_{r['id']}") and reject_reason != "—":
                        ...
                        # Return to PREALERT and record as a rejection event (audit)
                        ok, msg = apply_transition(
                            r,
                            "PREALERT",
                            user=selected_facility,
                            details={"reason": reject_reason, "facility": selected_facility},
                            action="CASE_REJECTED",
                            force=True,  # allow rejection from any state for demo
                        )
    ...
    if not active:
        ...
        with demo_col2:
            if st.button("Load Demo Case", key="load_demo_case_main"):
                ...
                demo_case = normalize_referral(demo_case)
        ...
        # Handle timeline button actions (state machine + audit)
        if accept_btn:
            ok, msg = apply_transition(r, "DISPATCHED", user="Ambulance/EMT", details={"tab": "ambulance"}, action="CASE_ACCEPTED", force=True)
            (st.success if ok else st.warning)(msg)
            st.rerun()
        elif enroute_btn:
            ok, msg = apply_transition(r, "ENROUTE_SCENE", user="Ambulance/EMT", details={"tab": "ambulance"})
            (st.success if ok else st.warning)(msg)
            st.rerun()
        elif onscene_btn:
            ok, msg = apply_transition(r, "ARRIVE_SCENE", user="Ambulance/EMT", details={"tab": "ambulance"})
            (st.success if ok else st.warning)(msg)
            st.rerun()
        elif depart_btn:
            ok, msg = apply_transition(r, "DEPART_SCENE", user="Ambulance/EMT", details={"tab": "ambulance"})
            (st.success if ok else st.warning)(msg)
            st.rerun()
        elif arrived_btn:
            ok, msg = apply_transition(r, "ARRIVE_DEST", user="Ambulance/EMT", details={"tab": "ambulance"})
            (st.success if ok else st.warning)(msg)
            st.rerun()
        elif handover_btn:
            ok, msg = apply_transition(r, "HANDOVER", user="Ambulance/EMT", details={"tab": "ambulance"})
            (st.success if ok else st.warning)(msg)
            st.rerun()
